...
const ImgMessage = ({ data }) => {
  ...
  return (
    ...
    <motion.div
      ...
      onDragEnd={startDrag}
      className={`message ${(data?.type === 2||data?.type === 5) && "img"} ${
    ...
    >
      ...
      {!zoomMode ? (
        <div  className="img-message" onClick={zoomImageMode}>
          {data.type === 5 && <p className="forward-sender-name" style={{padding: 10, paddingBottom: 0}}>Forward from <strong>{data?.forward?.user?.name}</strong></p>}
          <img src={data.type === 5 ? data.forward.file_url : data.file_url} alt="" draggable={false} />
          <p>{data.type === 5 ? data.forward.body : data.body}</p>
        </div>
      ) : (
        <div className="img-message" onClick={zoomImageMode}>
          <TransformWrapper>
