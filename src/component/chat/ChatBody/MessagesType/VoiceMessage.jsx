...
function VoiceMessage({ data }) {
  ...
  useEffect(() => {
    const wavesurfer = WaveSurfer.create({
      ...
      wavesurfer.load(`${getAudio}?file_name=${data.type === 5 ? data.forward.body :data.body}`);
    setWave(wavesurfer);
    wavesurfer.on("ready", () => {
      setTime(
  ...
  return (
    ...
    <motion.div
      ...
      className={`voice-message-container ${data.is_sender ? "end" : "start"}`}
      onContextMenu={handleRightClick}
    >
      <div className="message voice-message" style={{paddingTop: data.type === 5 && 30}}>
      {data.type === 5 && <p className="forward-sender-name">Forward from <strong>{data?.forward?.user?.name}</strong></p>}
      ...
      <OptionList setShowOptions={setShowOptions} setShowAlert={setShowAlert} data={data} showOptions={showOptions} />
    </motion.div>
    {
      showAlert && <Forward messageId={data.id} data={chats} Show={showAlert} off={setShowAlert}  />
    }
    </>
  );
}
