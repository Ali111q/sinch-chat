  ...
  return (
    <>
      <motion.div
        ...
        className={`message ${(data?.type === 1 || data?.type === 5) && "text"} ${
      ...
      >
        ...
        {data.type === 5 && <p className="forward-sender-name" style={{color: !data.is_sender && "rgb(0 0 0 / 69%)"}}>Forward from <strong>{data?.forward?.user?.name}</strong></p>}
        <h6>{data.type === 5 ? data.forward?.body :  data?.body}-({data.id})</h6>
        <span>{data?.time}</span>
        <OptionList data={data} setShowOptions={setShowOptions} setShowAlert={setShowAlert} showOptions={showOptions}/>
      </motion.div>
      {showAlert && (
        <Forward
          messageId={data.id}
