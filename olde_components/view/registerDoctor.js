...
import { regester } from '../utils/constant';
import reg from '../helper/register_helper';
  ...
  render() {
    ...
    return (
        ...
        <span class="add-name-sing">
            <p>الاسم الكامل</p>
            <input type="text" placeholder="اسم المستخدم" name="name" id=""  onChange={(val)=>{this.setState({Name :val.target.value} )
              } }/>
        </span>
        <span class="add-contry-sing">
            <p>رقم الهاتف</p>
            <div>
                <input type="tel" name="" id="" placeholder="ادخل رقم الهاتف" onChange={(val)=>{this.setState({mobile :val.target.value} )} }/>
                <button>
        ...
        <span class="cand">
            <h1>الجنس</h1>
            <select value="الجنس" name="cars" id="cars" onChange={(val)=>{this.setState({gender :val.target.value} )} } >
                <option value={1}>ذكر</option>
                <option value={2}>انثا</option>

            </select>
        </span>
        <span class="cand111">
            <h1>مواليد</h1>
            <input type="date" name="" id="" onChange={(val)=>{this.setState({date_of_birth :val.target.value} )} }/>
        </span>
        <span class="cand">
            <h1>المدينة</h1>
            <select value="" name="cars" id="cars" onChange={(val)=>{this.setState({city_id :val.target.value} )} }>
                <option value={1}>المدينه</option>
                <option value={2}>انثا</option>
        ...
        <span class="add-contry-sing">
            ...
            <div>
                <input type="texts" name="" id="" onChange={(val)=>{this.setState({date_of_birth :val.target.value} )} }/>
        ...
        <button class="sing-don" onClick={()=>{reg({
        mobile: "07713714113",
        account_type_id: 1,
        full_name:"ali", 
        country_id:1,
